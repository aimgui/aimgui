...
class Generator:
    ...
    def is_cursor_mappable(self, cursor):
        if cursor.location.file:
            return 'imgui_node_editor.h' in cursor.location.file.name
        return False
    ...
    def parse_enum(self, cursor):
        self.out(f'py::enum_<{self.name(cursor)}>(libaimbp, "{self.format_type(cursor.spelling)}", py::arithmetic())')
        self.out.indent += 1
        for value in cursor.get_children():
            #self.out(f'.value("{self.format_enum(value.spelling)}", {value.spelling})')
            self.out(f'.value("{self.format_enum(value.spelling)}", {self.name(value)})')
    ...
    def parse_class(self, cursor):
        if self.is_class_mappable(cursor):
            ...
            self.out(f'PYCLASS_BEGIN(libaimbp, {clsname}, {pyname})')
            ...
            self.out(f'PYCLASS_END(libaimbp, {clsname}, {pyname})\n')
