...
class Generator:
    ...
    def parse_enum(self, cursor):
        self.out(f'py::enum_<{self.name(cursor)}>(libaimplot, "{self.format_type(cursor.spelling)}", py::arithmetic())')
        self.out.indent += 1
    ...
    def parse_class(self, cursor):
        if self.is_class_mappable(cursor):
            ...
            self.out(f'PYCLASS_BEGIN(libaimplot, {clsname}, {pyname})')
            ...
            self.out(f'PYCLASS_END(libaimplot, {clsname}, {pyname})\n')
