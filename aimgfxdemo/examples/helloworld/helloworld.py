...
class HelloWorld(ExampleWindow):
    ...
    def init(self, platform_data):
        aimgfx.renderFrame()
        aimgfx.setPlatformData(platform_data)
        aimgfx.init(self.init_conf)

        aimgfx.setDebug(aimgfx.BGFX_DEBUG_TEXT)
        aimgfx.setViewClear(0, aimgfx.BGFX_CLEAR_COLOR | aimgfx.BGFX_CLEAR_DEPTH, 0x443355FF, 1.0, 0)
    ...
    def update(self, dt):
        ...
        aimgfx.setViewRect(0, 0, 0, self.width, self.height)
        aimgfx.touch(0)
        aimgfx.dbgTextClear(0, False)
        aimgfx.dbgTextImage(
        ...
        stats = aimgfx.getStats()

        aimgfx.dbgTextPrintf(
            1,
            1,
            0x0F,
