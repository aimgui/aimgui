...
class Overloaded(UserSet):
    ...
    def is_overloaded(self, cursor):
        if not hasattr(self.is_overloaded, 'overloaded'):
            self.is_overloaded.overloaded = set(self.overloaded)
        return self.name(cursor) in self.is_overloaded.overloaded
...
class Generator:
    ...
    @classmethod
    def create(self, filename="generate.toml"):
    ...
    def write_pyargs(self, arguments):
        for argument in arguments:
            ...
            if len(default):
                default = ' = ' + default
            out(', py::arg("{}"){}'.format(self.format_attribute(argument.spelling), default))
    ...
    def parse_overloads(self, cursor):
        if not hasattr(self.parse_overloads, 'visited'):
            self.parse_overloads.visited = set()
        if not hasattr(self.is_overloaded, 'overloaded'):
            self.is_overloaded.overloaded = set(self.overloaded)
        for child in cursor.get_children():
            if child.kind in [cindex.CursorKind.CXX_METHOD, cindex.CursorKind.FUNCTION_DECL]:
                key = self.name(child)
                if key in self.parse_overloads.visited:
                    self.is_overloaded.overloaded.add(key)
                else:
                    self.parse_overloads.visited.add(key)
