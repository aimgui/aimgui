...
class Generator:
    ...
    def is_cursor_mappable(self, cursor):
        if cursor.location.file:
            return 'imnodes.h' in cursor.location.file.name
        return False
    ...
    def parse_enum(self, cursor):
        self.out(f'py::enum_<{self.name(cursor)}>(libaimnodes, "{self.format_type(cursor.spelling)}", py::arithmetic())')
        self.out.indent += 1
    ...
    def parse_class(self, cursor):
        if self.is_class_mappable(cursor):
            ...
            self.out(f'PYCLASS_BEGIN(libaimnodes, {clsname}, {pyname})')
            ...
            self.out(f'PYCLASS_END(libaimnodes, {clsname}, {pyname})\n')
